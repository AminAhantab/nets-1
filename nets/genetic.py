...
def init_population(
    cls_name: str, num_params: int, pop_size: int, density: float = 1.0
) -> Tensor:
    """
    Initialise a population of individuals with random weights.

    Args:
        cls_name: Name of the class of model.
    ...
    if cls_name == "LeNetFeedForwardNetwork":
        for i in range(pop_size):
            ...
            population[i, 0, 300 * 28 * 28 : 300 * 28 * 28 + 300 * 100] = fc2.reshape(
                -1
            )
            population[i, 0, 300 * 28 * 28 + 300 * 100 :] = fc3.reshape(-1)

    elif cls_name == "ConvSixNeuralNetwork":
        for i in range(pop_size):
